        ...
        describe "part 2 solver" $ do
          it "gets the right answer for the example puzzle" $ do
            expectParsed
              ...
              ( \puzz -> do
                  solver2 puzz (getStartingNodes puzz) `shouldBe` 6
              )
        describe "findZs" $
          do
              ...
              do
                expectParsed
                  ...
                  ( \(Puzzle {nodes, steps}) -> do
                      findZs nodes steps "11A" 0 10 `shouldBe` [2, 4, 6, 8, 10]
                      findZs nodes steps "22A" 0 10 `shouldBe` [3, 6, 9]
                  )
            it "finds the right counts for a fourth example puzzle that doesn't cleanly repeat" $
              ...
              expectParsed parsedExamplePuzzle4 (\(Puzzle {nodes, steps}) -> findZs nodes steps "44A" 0 10 `shouldBe` [2, 4])
