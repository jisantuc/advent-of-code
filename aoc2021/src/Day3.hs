...
intLog x
  ...
  | otherwise = go 1 x
  where
    go n v = if sadExp 2 n > v then n - 1 else go (n + 1) x
...
findGammaBit puzz shift =
  let numberOfExamples = length puzz
      sumOfBits = foldl' (\acc n -> (shiftR n shift .&. 1 + acc)) 0 puzz
   in 2 * sumOfBits >= numberOfExamples
...
solvePart1 [] = 1
solvePart1 ns =
  let -- need a value with the most significant bit set
   ...
   in gammaSum * epsilonSum

solvePart2 :: Puzzle -> Maybe Int
solvePart2 [] = Nothing
solvePart2 ns =
  let maxValue = maximum ns
   ...
   in (*) <$> listToMaybe scrubberRating <*> listToMaybe oxygenRating
